<?php
// Ficheiro: app/Views/garcom/dashboard_api.php (Versão Definitiva e Corrigida)
...
<head>
    ...
    <style>
        /* O seu CSS existente continua aqui, sem alterações */
        :root {
        ...
        body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background-color: var(--cor-fundo); color: var(--cor-texto); margin: 0; padding: 20px; }
        .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
        .header h1 { color: var(--cor-primaria); }
        .logout-btn { background-color: var(--cor-perigo); color: white; padding: 10px 15px; border: none; border-radius: 5px; text-decoration: none; cursor: pointer; }
        .container { max-width: 1200px; margin: 0 auto; }
        #mesas-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 20px; margin-bottom: 30px; }
        .mesa { padding: 20px; border-radius: 8px; text-align: center; font-weight: bold; font-size: 1.2rem; color: white; cursor: pointer; transition: transform 0.2s, box-shadow 0.2s; box-shadow: var(--sombra); }
        .mesa:hover { transform: translateY(-5px); box-shadow: 0 8px 16px rgba(0,0,0,0.2); }
        .mesa[data-status="disponivel"] { background-color: var(--cor-sucesso); }
        .mesa[data-status="ocupada"] { background-color: var(--cor-perigo); }
        #pedidos-prontos-container { background: white; padding: 20px; border-radius: 8px; box-shadow: var(--sombra); }
        #pedidos-prontos-container h2 { margin-top: 0; color: var(--cor-primaria); }
        .pedido-pronto { display: flex; justify-content: space-between; align-items: center; padding: 15px; border-bottom: 1px solid #ddd; }
        .pedido-pronto:last-child { border-bottom: none; }
        .btn-entregar { background-color: var(--cor-secundaria); color: white; border: none; padding: 8px 12px; border-radius: 5px; cursor: pointer; }
        .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.5); }
        .modal-content { background-color: #fefefe; margin: 5% auto; padding: 30px; border: 1px solid #888; width: 90%; max-width: 600px; border-radius: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); }
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
             ...
             * A lógica para carregar e marcar pedidos como entregues foi totalmente reescrita
             * para funcionar por MESA, em vez de por PEDIDO.
             */
            const carregarPedidosProntos = async () => {
                const response = await apiFetch('/api/garcom/pedidos/prontos');
                if (!response?.success) return;
                
                pedidosProntosContainer.innerHTML = ''; // Limpa a lista
                ...
                // Cria uma notificação para cada MESA que tem pedidos prontos
                response.data.forEach(mesa => {
                    const pedidoDiv = document.createElement('div');
                    pedidoDiv.className = 'pedido-pronto';
                    
                    // O botão agora armazena o ID da MESA em um atributo 'data-mesa-id'
                    pedidoDiv.innerHTML = `
                        <span>Pedido da <strong>Mesa ${mesa.mesa_numero}</strong> está pronto!</span>
                        <button class="btn-entregar" data-mesa-id="${mesa.mesa_id}">Marcar como Entregue</button>
                    `;
                    pedidosProntosContainer.appendChild(pedidoDiv);
                });
            };

            // Adiciona um único 'event listener' ao container para lidar com todos os cliques
            pedidosProntosContainer.addEventListener('click', async (event) => {
                // Verifica se o elemento clicado é um botão de 'entregar'
                if (event.target.classList.contains('btn-entregar')) {
                    const mesaId = event.target.dataset.mesaId;
                    if (!mesaId) return;

                    // Chama a API enviando o ID da MESA
                    ...
                    // Se a operação for bem-sucedida, atualiza o dashboard para a notificação desaparecer
                    if (response?.success) {
                        atualizarDashboard();
                    }
                }
            ...
            const renderizarDetalhesPedido = (pedido) => {
                ...
                if (pedido?.itens?.length > 0) {
                    ...
                    pedido.itens.forEach(item => {
                        ...
                        total += itemTotal;
                        itensList.innerHTML += `<li><span>${item.quantidade}x ${item.nome}</span> <span>R$ ${itemTotal.toFixed(2)}</span></li>`;
                    });
            ...
            const carregarCardapio = async () => { /* ... seu código ... */ };
            const renderizarCardapio = (categorias) => { /* ... seu código ... */ };
            formNovoPedido.addEventListener('submit', async (e) => { /* ... seu código ... */ });
            const fecharModal = () => { /* ... seu código ... */ };
            closeModalBtn.onclick = fecharModal;
            window.onclick = (event) => { if (event.target == modal) fecharModal(); };
            btnAdicionarItens.onclick = async () => { /* ... seu código ... */ };
